...
Then each weight is quantized by `round(w / s)` and clamped to the target integer range `example 4bit (-7..,0,..7)`, and now we are searching for the best scale_multiplier instead of thresholds like previously.
